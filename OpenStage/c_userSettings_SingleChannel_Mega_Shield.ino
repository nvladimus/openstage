...
// This file contains global definitions that need to be modified by the user for the stage to 
// work correctly. 


// This file is set up for a single-axis and which would work with both the Uno (only low
// pin numbers are used here) or the Mega. The LCD and game pad are disabled. You may modify the
// definitions below to suit your hardware. 
...
byte stepOut[maxAxes]={9,24,26,0}; //Set these to the step out pins (ordered X, Y, and Z)

// stepDir
// These pins tell the Big Easy Driver to which they connect which direction to rotate the motor
byte stepDir[maxAxes]={8,25,27,0}; //Ordered X, Y, and Z
...
byte enable[maxAxes]={13,29,30,0}; //Ordered X, Y, and Z

// The microstep pins.
// These pins define the microstep size. The MS pins on all axes are wired together.
byte MS[4][3]={ 
	{12,11,10},  //channel 1, MS pins 1,2,3
...
// Define rotary encoder pins, they should have Interrupt capability, see https://www.pjrc.com/teensy/td_libs_Encoder.html
  ...
  Encoder rotEncoderX(18,19); //pins (2,3) do not work on Mega for some reason.
  byte rotEncoderPushButton = 4; //pin for a push button input
  byte rotEncoderLEDs[3] = {5,6,7}; //LED pins, LOW for turning ON.
...
bool axisPresent[maxAxes]={0,0,0,0}; //don't edit
